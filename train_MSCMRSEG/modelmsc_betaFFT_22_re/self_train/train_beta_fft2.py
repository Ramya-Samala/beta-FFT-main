...
parser.add_argument("--root_path", type=str, default="../data/MS_CMRSEG/mscmrseg19_split2", help="Name of Experiment")
parser.add_argument("--exp", type=str, default="msc02/diffrect", help="experiment_name")
...
def pre_train(args, snapshot_path):
    ...
    labeled_slice = patients_to_slices("mscmrseg19_split2", args.labeled_num)
...
def train(args,pre_snapshot_path, snapshot_path):
    ...
    if 'state_dict' in torch.load(pre_trained_model, map_location=device).keys():
        ema_model.load_state_dict(torch.load(pre_trained_model, map_location=device)['state_dict'])
        model_1.load_state_dict(torch.load(pre_trained_model, map_location=device)['state_dict'])
        model_2.load_state_dict(torch.load(pre_trained_model, map_location=device)['state_dict'])
    else:
        ema_model.load_state_dict(torch.load(pre_trained_model, map_location=device))
        model_1.load_state_dict(torch.load(pre_trained_model, map_location=device))
        model_2.load_state_dict(torch.load(pre_trained_model, map_location=device))
...
#             ICT
            ...
            # Mix images
            batch_ux_mixed_s = uimg_a_s* \
                (1.0 - mix_factors_s) + \
                uimg_a_s * mix_factors_s
            ...
            with torch.no_grad():
                ...
                ema_output_ux1_s = torch.softmax(
                    pre_b_s, dim=1)
                batch_pred_mixed_s = ema_output_ux0_s * \
                    (1.0 - mix_factors_s) + ema_output_ux1_s * mix_factors_s
...
if __name__ == "__main__":
    if args.deterministic:
        ...
        # -- path to save models
    pre_snapshot_path = "./modelmsc_betaFFT_22_re/pre_train"
    self_snapshot_path = "./modelmsc_betaFFT_22_re/self_train"
    for snapshot_path in [pre_snapshot_path, self_snapshot_path]:
        if not os.path.exists(snapshot_path):
            os.makedirs(snapshot_path)
    shutil.copy('./train_beta_fft2.py', self_snapshot_path)
