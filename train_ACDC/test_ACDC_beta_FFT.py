...
parser.add_argument('--exp', type=str, default='beta_FFT', help='experiment_name')
...
parser.add_argument('--model_path', type=str, default='../best_results/ACDC_5/unet_best.pth', help='path to model checkpoint')
...
def Inference(FLAGS,model_path):
    ...
    saved_state_dict = torch.load(save_model_path, map_location=device)
    
    # Filter out incompatible keys (projection_head, prediction_head, contrastive_class_selector)
...
if __name__ == '__main__':
    ...
    metric = Inference(FLAGS, FLAGS.model_path)
    print("Results per class:")
    print(metric)
    print("Average Dice Score:")
    print((metric[0][0]+metric[1][0]+metric[2][0])/3)
    print("Average Jaccard Score:")
    print((metric[0][1]+metric[1][1]+metric[2][1])/3)
    print("Average HD95 Score:")
    print((metric[0][2]+metric[1][2]+metric[2][2])/3)
    print("Average ASD Score:")
    print((metric[0][3]+metric[1][3]+metric[2][3])/3)
