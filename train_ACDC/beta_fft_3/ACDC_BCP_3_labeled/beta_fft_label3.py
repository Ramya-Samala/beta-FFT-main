...
def get_ACDC_2DLargestCC(segmentation):
    ...
    device = get_device()
    # Convert list of numpy arrays to a single numpy array first to avoid warning
...
#       #Mix   Just for student1
            ...
            mix_factors_s = torch.tensor(
                mix_factors_s, dtype=torch.float).to(device)
...
#             FFT
            ...
            logging.info('iteration %d: loss: %f, ICT_loss: %f, FFT_loss: %f' % (iter_num, loss,  consistency_loss +supervised_loss_mix  , pseudo_supervision5+pseudo_supervision6))
            logging.info('iteration %d: loss: %f, pseudo_supervision5: %f, pseudo_supervision6: %f' % (iter_num, loss, pseudo_supervision5, pseudo_supervision6))
            if iter_num > 0 and iter_num % 200 == 0:
                model_1.eval()
                metric_list = 0.0
                for _, sampled_batch in enumerate(valloader):
...
#                 测试
                ...
                writer.add_scalar('info/model1_test_mean_dice', performance2_test, iter_num)
...
if __name__ == "__main__":
    ...
    self_snapshot_path = "./beta_fft_3/ACDC_{}_{}_labeled/".format(args.exp, args.labelnum)
    for snapshot_path in [pre_snapshot_path, self_snapshot_path]:
        if not os.path.exists(snapshot_path):
            os.makedirs(snapshot_path)
    shutil.copy('./beta_fft_label3.py', self_snapshot_path)
    # Pre_train
